...
public class ThornBoltEntity extends AbstractArrow implements ItemSupplier {
	...
	public ThornBoltEntity(EntityType<? extends ThornBoltEntity> type, double x, double y, double z, Level world, @Nullable ItemStack firedFromWeapon) {
		super(type, x, y, z, world, PROJECTILE_ITEM, firedFromWeapon);
	}

	public ThornBoltEntity(EntityType<? extends ThornBoltEntity> type, LivingEntity entity, Level world, @Nullable ItemStack firedFromWeapon) {
	...
	public static ThornBoltEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		ThornBoltEntity entityarrow = new ThornBoltEntity(ArsFaunaModEntities.THORN_BOLT.get(), entity, world, null);
		...
		world.playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("enchant.thorns.hit")), SoundSource.PLAYERS, 1, 1f / (random.nextFloat() * 0.5f + 1) + (power / 2));
		return entityarrow;
	}

	public static ThornBoltEntity shoot(LivingEntity entity, LivingEntity target) {
		ThornBoltEntity entityarrow = new ThornBoltEntity(ArsFaunaModEntities.THORN_BOLT.get(), entity, entity.level(), null);
		...
		entity.level().playSound(null, entity.getX(), entity.getY(), entity.getZ(), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("enchant.thorns.hit")), SoundSource.PLAYERS, 1, 1f / (RandomSource.create().nextFloat() * 0.5f + 1));
		return entityarrow;
	}
}
